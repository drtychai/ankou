...
pub fn log(_args: Vec<String>, max_parents: usize, min_parents: usize) -> Result<(), Error> {
    ...
    //}
    //let ps = Pathspec::new(&Vec::new().iter())?; //args.arg_spec.iter())?;
    ...
    let revwalk = revwalk
        .filter_map(|id| {
            ...
            if parents < min_parents {
                return None;
            }
            if parents >= max_parents {
                return None;
            }
            //if let n = max_parents {
            //    if parents >= n {
            //        return None;
            //    }
            //}
            if !false {
                ...
                match commit.parents().len() {
                    0 => {
                        ...
                        let _flags = git2::PathspecFlags::NO_MATCH_ERROR;
        ...
        .skip(0) //args.flag_skip.unwrap_or(0))
        .take(!0); //args.flag_max_count.unwrap_or(!0));
...
fn _log_message_matches(msg: Option<&str>, grep: &Option<String>) -> bool {
    match (grep, msg) {
        (&None, _) => true,
        (&Some(_), None) => false,
